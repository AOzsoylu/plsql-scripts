...
rem
set feedback off
column DBSID new_value DBSID
select substr(substr(global_name,1,30),1,instr(substr(global_name,1,30),'.')-1) DBSID
  from global_name;
prompt
...
prompt the init&DBSID..ora file.
set heading off
...
prompt SHARED_POOL_SIZE in the init&DBSID..ora file.
set heading off
select 'Current SHARED_POOL_SIZE value is ' || value from v$parameter where name = 'shared_pool_size';
set heading on
...
prompt REQUEST_FAILURES, so increase SHARED_POOL_RESERVED_SIZE in the init&DBSID..ora
...
prompt parameter in the init&DBSID..ora file.
set heading off
select 'Current SHARED_POOL_SIZE value is ' || value from v$parameter where name = 'shared_pool_size';
set heading on
...
prompt for DB_BLOCK_BUFFERS in the init&DBSID..ora file.
set heading off
...
prompt increasing the DB_BLOCK_BUFFERS parameter in the init&DBSID..ora file.
set heading off
select 'Current DB_BLOCK_BUFFERS value is ' || value from v$parameter where name = 'db_block_buffers';
set heading on
...
prompt DB_WRITER_PROCESSES parameter in the init&DBSID..ora file.
set heading off
...
prompt in the init&DBSID..ora file.
set heading off
...
prompt in the init&DBSID..ora file if the selection below doesn't show "no rows selected".
set heading off
...
prompt increasing the SORT_AREA_SIZE parameter in the init&DBSID..ora file. You
...
prompt ================================================================================;
prompt
prompt .                      Tuning Latches
prompt
prompt ================================================================================;
prompt
prompt if miss_ratio or immediate_miss_ratio > 1 then latch 
prompt contention exists, decrease LOG_SMALL_ENTRY_MAX_SIZE in init&DBSID..ora
prompt 
column name format a40
column "miss_ratio" format 9990.99 
column "immediate_miss_ratio" format 9990.99 
